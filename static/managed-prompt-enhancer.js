(() => {
  ...
  function ensurePanel() {
    ...
    if (!provider.querySelector('option[value="managed"]')) {
      const option = document.createElement('option');
      option.value = 'managed';
      option.textContent = 'Managed Local — Gemma 4 E2B (Private)';
    ...
    if (!panel) {
      ...
      panel.className = 'hidden bg-zinc-950 border border-zinc-800 rounded-2xl p-4 space-y-3';
    ...
    return `
      <div class="space-y-2">
        <div class="h-2 rounded-full bg-zinc-800 overflow-hidden">
          <div class="h-full bg-orange-500 transition-all duration-300" style="width:${total ? percent : 4}%"></div>
        </div>
  ...
  function renderStatus(status) {
    ...
    else if (installing) badge = '<span class="text-[10px] uppercase tracking-wider font-bold text-orange-400">Installing</span>';
    ...
    panel.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        <div>
          <div class="flex items-center gap-2">
            ...
            <strong class="text-sm text-zinc-200">Managed Local</strong>
            ${badge}
          </div>
          <p class="text-xs text-zinc-500 mt-1">${model.name || 'Gemma 4 E2B Instruct'} · ${model.quantization || 'Q4_0 QAT'} · ~3.35 GB</p>
          <p class="text-[11px] text-zinc-600 mt-1">Private llama.cpp runtime on this computer. CPU-first so ComfyUI keeps the GPU; the model unloads from memory after 5 idle minutes.</p>
        </div>
      </div>
      ...
      <div class="flex flex-wrap gap-2">
        ${!installed && supported ? `<button id="managed-enhancer-install" ${installing ? 'disabled' : ''} class="bg-orange-600 hover:bg-orange-500 disabled:opacity-40 text-white px-3 py-2 rounded-lg text-xs font-semibold transition flex items-center gap-2"><i data-lucide="${failed ? 'rotate-ccw' : 'download'}" class="w-3.5 h-3.5"></i>${failed ? 'Retry Install' : 'Install Gemma 4'}</button>` : ''}
        ...
        ${installed ? '<button id="managed-enhancer-remove" class="bg-zinc-900 hover:bg-red-950/40 border border-zinc-800 hover:border-red-900 text-zinc-500 hover:text-red-300 px-3 py-2 rounded-lg text-xs font-medium transition">Remove local model</button>' : ''}
      </div>
      ${!supported ? `<div class="text-xs text-amber-300">Managed Local is not packaged for ${status.platform || 'this platform'} yet. Existing Ollama and API providers still work.</div>` : ''}
  ...
  async function installManaged() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.detail || 'Could not start Managed Local installation.');
      renderStatus(data);
    } catch (error) {
  ...
  async function testManaged() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || 'Managed Local test failed.');
      if (target) target.textContent = `✓ ${data.response || 'Managed Local is ready.'}`;
      await refreshStatus();
    } catch (error) {
      if (target) target.textContent = error.message;
    }
  ...
  async function removeManaged() {
    if (!confirm('Remove Orange\'s managed Gemma 4 model and llama.cpp runtime? This does not affect Ollama, LM Studio, or ComfyUI models.')) return;
    try {
      const response = await authFetch('/api/admin/prompt-enhancer/managed/remove', {method: 'POST'});
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || 'Could not remove Managed Local.');
      renderStatus(data);
    } catch (error) {
      const target = document.getElementById('managed-enhancer-action-status');
      if (target) target.textContent = error.message;
