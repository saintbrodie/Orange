...
class WorkflowPackAdminTests(unittest.TestCase):
    ...
    def test_install_adds_tool_only_after_routable_preflight(self):
        ...
        install_result = {
            "failures": [],
            "selectedModels": [{"filename": "krea2_turbo_int8_convrot.safetensors", "precision": "int8"}],
        ...
        updated_config = {**config, "tools": [*config["tools"], manifest["tool"]]}
        ...
        async def run_test():
            with mock.patch.object(workflow_pack_admin, "get_workflow_pack", return_value=manifest), mock.patch.object(
                ...
                workflow_pack_admin, "_system_stats", new=mock.AsyncMock(return_value={"devices": []})
            ), mock.patch.object(workflow_pack_admin, "install_workflow_pack", return_value=install_result), mock.patch.object(
